...
export function ChannelsView() {
  const { channels, toggleChannel, setView } = useApp();
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Link the places where PingFlow should send alerts. You can add or remove
            channels at any time from settings.
          </p>
        </div>
        ...
        <ChannelCard
          ...
          action={
            channels[0].connected ? (
              <button
                onClick={() => toggleChannel("whatsapp", false)}
        ...
        <ChannelCard
          ...
          action={
            channels[1].connected ? (
              <button
                onClick={() => toggleChannel("messenger", false)}
                className="btn-secondary"
              >
                Disconnect
              </button>
      ...
      <div className="mt-10 flex flex-col items-center justify-between gap-4 rounded-xl border border-border bg-card p-5 shadow-card sm:flex-row">
        ...
        <button onClick={() => setView("tasks")} className="btn-primary !w-auto px-6">
          Continue to Tasks →
        </button>
      </div>
